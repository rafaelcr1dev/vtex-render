...
"function"!==typeof String.prototype.replaceSpecialChars&&(String.prototype.replaceSpecialChars=function(){var b={"\u00e7": "c", 
"\u00e6": "ae", 
"\u0153": "oe", 
"\u00e1": "a", 
"\u00e9": "e", 
...
return this.replace(/[\u00e0-\u00fa]/g, function(a){return "undefined"!=typeof b[a]?b[a]:a;});});
...
jQuery.fn.vtexSmartResearch=function(opts)
{
	...
	if ($("a.vejamais-vtexresearch").length == 0){
		$('<div class="products-actions"><a href="#" class="vejamais-vtexresearch btn btn--outline">CARREGAR MAIS PRODUTOS</a></div>').insertAfter(".pager.bottom");
	}
    ...
    var defaults=
	{
		...
		ajaxCallback: function(d){
			...
			setTimeout(function(){
             if ($(".products:eq(0) .prateleira ul li:not(.helperComplement)").length <= 14){
                $(".pager.bottom").hide();
             } else {
             	$(".pager.bottom").hide();
             }
	...
	var fn=
	{
		...
		infinitScroll: function(type)
		{
			...
			if (type && infinitScroll == false){
				...
				$("body").on('click', 'a.vejamais-vtexresearch', function(e){
						...
						if (currentStatus)
						{
							...
							pageJqxhr=jQuery.ajax({
								...
								success: function(data)
								{
									...
									{
                    moreResults=false;
                    alert("Não foram encontrados mais produtos!");
                    log("Não existem mais resultados a partir da página: "+(currentPage-1), "Aviso");
                    $("a.vejamais-vtexresearch").addClass('finished').hide();
									}
									else
		...
		{	
			...
			filtersMenuE.find("fieldset").each(function(){
				...
				label.each(function(ndx){
					...
					t.addClass(labelClass).attr({"title": v, 
"index": ndx});
		...
		filterAjaxSuccess: function(data)
		{
			...
			ajaxCallbackObj.requests++;
			options.ajaxCallback(ajaxCallbackObj);
			_html.animate({scrollTop: options.filterScrollTop((loadContentOffset||{top: 0, 
left: 0}))}, 600);
		},
		...
		updateContent: function($data)
		{
			...
			(shelfPage.length>0?shelfPage:options.emptySearchElem).slideUp(50, function(){
				jQuery(this).remove();
